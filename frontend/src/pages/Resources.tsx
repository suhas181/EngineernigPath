...
import { trackView, formatViewCount } from '../services/viewTrackingService';
...
export function Resources() {
  ...
  const [sortByMostViewed, setSortByMostViewed] = useState(false);
  ...
  const displayedResources = useMemo(() => {
    ...
    return resources;
  }, [resources, sortByMostViewed]);

  const hasActiveFilters = search.trim() !== '' || selectedCategory !== 'All' || selectedLanguage !== 'all' || bookmarkedOnly || sortByMostViewed;
    ...
    <MosaicShell>
      <div className="space-y-8 pb-20 text-left">
        ...
        <div className="flex items-center space-x-2 overflow-x-auto pb-2 scrollbar-none">
          ...
          <button
            onClick={() => setSortByMostViewed(!sortByMostViewed)}
            className={`px-4 py-2 rounded-xl text-xs font-bold transition-all duration-200 flex items-center space-x-1.5 shrink-0 border ${
              sortByMostViewed
                ? 'bg-rose-600 text-white border-rose-500 shadow-md shadow-rose-500/20'
                : 'bg-white/80 dark:bg-white/5 border-slate-200 dark:border-white/10 text-slate-700 dark:text-slate-300 hover:bg-slate-50 dark:hover:bg-white/10'
            }`}
            title="Sort resources by highest view count"
          >
...
function ResourceCard({
  ...
  const [localViews, setLocalViews] = useState<number>(resource.views || 0);

  useEffect(() => {
    if (typeof resource.views === 'number') {
      setLocalViews(resource.views);
    }
  }, [resource.views]);
      ...
      <div className="p-4 flex-1 flex flex-col justify-between space-y-3">
        <div className="space-y-1.5">
          ...
          <div className="flex items-center justify-between text-[11px] pt-1.5 border-t border-slate-100">
            ...
            <div className="flex items-center space-x-2">
              <span className="inline-flex items-center space-x-1 px-2 py-0.5 rounded-full bg-slate-100 text-slate-600 text-[10px] font-semibold border border-slate-200/60" title={`${localViews} views`}>
                <Eye className="w-3 h-3 text-slate-400" />
                <span>{formatViewCount(localViews)} views</span>
              </span>
              {resource.provider === 'ResuMax' && (
